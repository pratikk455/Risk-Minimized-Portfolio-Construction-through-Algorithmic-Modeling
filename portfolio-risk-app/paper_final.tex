...
\begin{abstract}
Portfolio optimization remain a fundamental challenge in the quantitative finance, yet institutional-quality tools remain largely inaccessible to the retail investors. This paper present a comprehensive portfolio management system that democratize sophisticated optimization strategies while providing educational value. The system combine Hierarchical Risk Parity (HRP) and Modified Markowitz Mean-Variance Optimization in novel hybrid approach, driven by 28-question risk assessment that generate truly personalized portfolios rather than categorical allocations. Unlike traditional robo-advisors that map users into discrete risk categories, our approach produce continuous spectrum of portfolios—theoretically $10^{28}$ unique combinations—ensuring each user receive allocations tailored to their specific risk capacity, behavioral tolerance, and thematic preferences. Backtesting on 2019-2024 market data demonstrate that hybrid approach achieve Sharpe ratio of 0.97 compared to 0.93 for the S\&P 500, while reducing maximum drawdown from 33.7\% to 26.7\%. The complete system, including paper and live trading integration via Alpaca Markets API, is deployed on Google Cloud Run and available as open source, representing concrete step toward democratizing institutional-grade investment strategies.
\end{abstract}
...
Proposed system create full-fledged portfolio management platform that democratize institutional-quality strategies while offering educational value. System evaluate personal risk profiles using comprehensive 28-question assessment, develop diversified portfolios across multiple ETF asset classes using Hierarchical Risk Parity and Markowitz optimization, focus on reducing risk rather than maximizing returns, provide interactive learning platform with simplified metric explanations, and enable both paper and real trading through Alpaca API.


\section{Related Work}

In modern financial theory, portfolio optimization is a well-known concept and therefore used in asset the management and quantitative finance industry. These companies try to find the best way to allocate assets while meeting the expected returns and maintaining the risks.
...
Foundation of any portfolio optimization system lie in the quality and diversity of its underlying data. For this project, I assembled comprehensive dataset that span multiple asset classes, drawing from both market data sources and user-generated behavioral information. Goal was to create system that not only understand the mathematical relationships between assets but also capture the nuanced preferences and risk tolerances of individual investors.

\subsection{ETF Universe Selection}

Rather than attempting to optimize across thousands of individual securities, which would introduce unnecessary complexity and potential overfitting, I curated focused universe of twenty Exchange-Traded Funds (ETFs) that provide broad market exposure across distinct asset categories. This approach align with democratization goal of the project—retail investors benefit more from well-diversified ETF portfolio than from attempting to pick individual stocks, a practice that even professional fund managers struggle to do consistently.

Stock category include eight ETFs spanning different investment styles and geographic regions. Bond category comprise five ETFs representing fixed-income spectrum. Alternative investments round out the universe with three ETFs providing exposure to non-traditional asset classes. Table~\ref{tab:etf-universe} present complete ETF universe.
...
Short-term government bonds receive lowest risk rating, while cryptocurrency and high-growth technology funds receive highest. These risk levels serve as initial filter—conservative investors will not be allocated to high-risk assets regardless of their potential returns.

\subsection{Market Data Sources}

Historical price data form the backbone of optimization algorithms, enabling calculation of expected returns, volatilities, and crucially, the correlation structure between assets. I utilize Yahoo Finance API~\cite{yahoofinance2025} through \texttt{yfinance} Python library to retrieve five years of daily adjusted closing prices for each ETF. This timeframe capture multiple market regimes, including COVID-19 crash of March 2020 and subsequent recovery, 2022 bear market driven by inflation concerns, and 2023-2024 technology-led rally.

From this historical data, I compute annualized volatility for each ETF as standard deviation of daily returns multiplied by square root of 252 (typical number of trading days per year). Expected returns are derived from combination of historical performance and forward-looking analyst estimates, acknowledging that past performance does not guarantee future results but provide reasonable baseline for optimization.

Correlation matrix, which capture how assets move together, is constructed from pairwise correlations of daily returns across five-year period. This matrix is fundamental to both optimization algorithms—Hierarchical Risk Parity use it to cluster similar assets, while Markowitz optimization use derived covariance matrix to identify portfolios on efficient frontier. Figure~\ref{fig:correlation} visualize correlation structure across representative asset classes.
...
Beyond static data, system retrieve real-time quotes from Yahoo Finance for portfolio valuation and trade execution. When user view their portfolio or initiate a trade, current market prices ensure accurate position sizing and profit/loss calculations. For actual trade execution, system integrate with Alpaca Markets API, which provide commission-free trading for both paper (simulated) and live accounts.

\subsection{User Assessment Data}

Traditional robo-advisors typically ask five to ten questions and map users into broad categories like ``Conservative,'' ``Moderate,'' or ``Aggressive.'' This approach fail to capture nuanced preferences—two individuals might both tolerate moderate risk, but one might prefer technology stocks while other prefer dividend income. Assessment I developed comprise twenty-eight questions organized into six dimensions, as shown in Table~\ref{tab:assessment-dimensions}.
...
Each response is recorded on scale from 1 to 10, creating continuous rather than categorical profile. This produce theoretical space of $10^{28}$ unique user profiles—ensuring truly personalized portfolios rather than predefined buckets.
...
\begin{equation}
\alpha_{\text{HRP}} = \max(0.1, 0.6 - 0.06 \times S)
\end{equation}
\begin{equation}
w_i^{\text{hybrid}} = \alpha_{\text{HRP}} \cdot w_i^{\text{HRP}} + (1 - \alpha_{\text{HRP}}) \cdot w_i^{\text{Markowitz}}
\end{equation}

Table~\ref{tab:blend-examples} show example blending weights.
...
\subsection{Diversification Improvements}
...
\begin{table}[H]
\centering
\caption{Diversification Comparison}
\label{tab:diversification}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Generated Portfolios} & \textbf{S\&P 500} \\
\midrule
Max single holding & 20--35\% & 7\% (Apple) \\
Top 7 holdings & 40--55\% & $\sim$30\% (Mag 7) \\
...
Backtesting on 2019--2024 historical data using actual ETF prices validate the hybrid approach. While pure HRP and Markowitz optimize heavily for risk reduction (resulting in bond-heavy portfolios), hybrid approach achieve near-market returns with significantly reduced drawdown.

\begin{table}[H]
\centering
\caption{Algorithm Performance Comparison (Moderate Risk, 2019--2024)}
...
Sharpe Ratio & 0.43 & 0.75 & \textbf{0.97} & 0.93 \\
Maximum Drawdown & -15.3\% & -14.7\% & -26.7\% & -33.7\% \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:backtest} visualize cumulative performance over backtesting period, showing how hybrid approach provide competitive returns while significantly dampening losses during market stress events.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{paper_figures/backtest_performance.pdf}
\caption{Cumulative portfolio performance comparison (2019--2024). Shaded regions indicate market stress periods. The hybrid approach achieves competitive returns while reducing volatility.}
\label{fig:backtest}
\end{figure}
...
However, this limitation highlight important insight: no strategy outperform in every market regime. Hybrid approach's value lie in its superior \textit{risk-adjusted} performance across full market cycle, achieving higher Sharpe ratio (0.97 vs.\ 0.93) and significantly lower maximum drawdown (-26.7\% vs.\ -33.7\%) than benchmark over complete 2019--2024 period. Figure~\ref{fig:drawdown} show drawdown profile over time, illustrating how diversified portfolios recover faster from market shocks.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{paper_figures/drawdown_comparison.pdf}
\caption{Maximum drawdown comparison over the backtesting period. The hybrid portfolio's maximum drawdown of -26.7\% compares favorably to the S\&P 500's -33.7\% during COVID.}
\label{fig:drawdown}
\end{figure}

\subsection{Risk-Return Analysis}

Figure~\ref{fig:riskreturn} show risk-return characteristics of each strategy, illustrating tradeoff between volatility reduction and return. Hybrid approach occupy favorable position in risk-return space, achieving 93\% of S\&P 500's returns while reducing volatility by 11\% and maximum drawdown by 21\%. Figure~\ref{fig:allocation} demonstrate how asset allocation shift across risk spectrum using actual optimizer, from bond-heavy conservative portfolios to equity-dominated aggressive allocations.
...
\noindent Smooth transition from conservative to aggressive allocations demonstrate that optimizer respond predictably to changes in user risk preferences, avoiding discontinuous jumps that could confuse users or produce inconsistent portfolios.

\subsection{Monte Carlo Simulation}

To validate robustness of hybrid optimization approach beyond single portfolio configuration, I conducted Monte Carlo simulation generating 1,000 portfolios with randomly varied risk assessment inputs. Each simulation used randomly generated responses (values 1--10) for all 16 assessment dimensions, producing portfolios spanning full spectrum of risk tolerances and investment preferences.
